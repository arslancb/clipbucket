...
if(isset($_POST['link_selected'])){
	...
	if ($cnt>0){
		for($id=0;$id<$cnt;$id++)
			$linkquery->unlink_link($_POST['checked_links'][$id],$video['videoid']);
	}
	//add selected link from the second list
	$cnt=count($_POST['check_links']);
	if ($cnt>0){
		for($id=0;$id<$cnt;$id++)
			$linkquery->link_link($_POST['check_links'][$id],$video['videoid']);
	}
}
...
$linkedLinks = $linkquery->get_link_for_video($result_array);
if ($filtercond) $result_array['cond']=$filtercond;
$result_array['selected'] = 'no';
$result_array['assign'] = 'unlinkedLinks';
$unlinkedLinks = $linkquery->get_link_for_video($result_array);


//Collecting Data for Pagination
$mcount = $array;
$mcount['count_only'] = true;
$total_rows  = $linkquery->get_link_for_video($mcount);
